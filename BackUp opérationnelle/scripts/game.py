...
class Game:
    ...
    # def load_interest_map(self, map_id):
    #     """ Load a map """
    #     json_filename = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "resources", "config.json")
    #     with open(json_filename, "r") as json_file:
    #         self.map_cfg = json.load(json_file)[f"map_{map_id}"]        

    #     size_x, size_y = self.map_cfg["width"], self.map_cfg["height"]

    #     final_mesh = np.zeros(shape=(size_x, size_y))
    ...
    #         # Set the interest point
    #         interest_point = [key.x, key.y]

    #         # Calculate values as a gradient from the interest point
    #         distance = np.sqrt((X - interest_point[0])**2 + (Y - interest_point[1])**2)
    #         gradient_values = 1 / (1 + distance)
    #         final_mesh = final_mesh + gradient_values
    ...
    #     final_mesh = np.rot90(final_mesh, k=2)
    #     # Plot the 2D array (map)
    #     plt.imshow(final_mesh, cmap='viridis', origin='lower', extent=[0, size_x-1, 0, size_y-1])
    #     plt.colorbar()
    #     plt.title('2D Array with Gradient Values from Interest Point')
    #     plt.xlabel('X-axis')
    #     plt.ylabel('Y-axis')
    #     plt.figure()
    #     plt.imshow(self.map_real)
    #     plt.show()
